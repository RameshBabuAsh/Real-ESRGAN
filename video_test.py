...
# ---------------------------
# Process and Display Frames Side by Side
# ---------------------------
while True:
    ...
    frame_test = cv2.resize(frame_test, (320, 180))
    frame_org  = cv2.resize(frame_org,  (320, 180))
    
    # Apply preprocessing to the test frame
    preprocessed = preprocess_frame(frame_test)
    
    # Convert preprocessed frame to RGB, normalize and to tensor
    ...
    # Convert original frame from BGR to RGB for display
    org_display = cv2.cvtColor(frame_test, cv2.COLOR_BGR2RGB)
    
    # Concatenate model output and original side by side
    ...
    cv2.imshow("Model Output (Left) vs Original (Right)", combined_bgr)
